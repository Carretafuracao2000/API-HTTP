...
if (!novaTarefa.titulo){
    ...
    resposta.end(JSON.stringify({error: 'O campo "titulo" é obrigatório'}))
}
...
resposta.statusCode = 400
resposta.end(JSON.stringify({errer:'Formato JSON inválido!'}))
    }
})    
} else {
    resposta.statusCode = 404
    resposta.end(JSON.stringify({error: 'Página mão encontrada'}))
}
});
